...
from .models import AwsClaimOptions
from .output import check_payload, claim_payload, cleanup_payloads, format_check_line, format_check_summary, format_claim_line, format_cleanup_line
...
def print_services(payload: dict[str, dict[str, Any]], *, json_output: bool = False, color: bool = False) -> None:
    ...
    emit(log_line("INF", f"aws services: {check_count} check handlers, {claim_count} claim handlers", color=color))
...
def format_precheck_line(payload: dict[str, Any], *, color: bool = False) -> str:
    if payload["ok"]:
        detail = payload["account"] or payload["arn"] or "authenticated"
        return log_line("INF", f"aws precheck: ok {tag_join(detail, color=color)}", color=color)
    return log_line("ERR", f"aws precheck: failed {compact_message(payload['message'])}", color=color)


def run_precheck_once(*, region: str | None, profile: str | None, json_output: bool, color: bool, silent_success: bool = False) -> bool:
    payload = precheck_payload(precheck(region=region, profile=profile))
...
def add_command_parsers(parser: argparse.ArgumentParser) -> None:
    ...
    precheck_parser = subcommands.add_parser("precheck", help="Check AWS CLI and credentials")
    add_color_args(precheck_parser)
    add_common_args(precheck_parser, defaults=False)
    ...
    check = subcommands.add_parser("check", help="Classify AWS hostnames and check claimability")
    add_color_args(check)
    add_common_args(check, defaults=False)
    check.add_argument("inputs", nargs="+", help="AWS hostnames or .txt input files")
    ...
    claim = subcommands.add_parser("claim", help="Claim supported available AWS hostnames")
    add_color_args(claim)
    add_common_args(claim, defaults=False)
    claim.add_argument("inputs", nargs="+", help="AWS hostnames or .txt input files")
    ...
    claim.add_argument("--application-name", default="cloudclaim-eb", help="Elastic Beanstalk application name")
    claim.add_argument("--solution-stack-name", help="Elastic Beanstalk solution stack. Auto-selected if omitted.")
    claim.add_argument("--cleanup", action="store_true", help="Terminate claimed proof environments after claiming")
    claim.set_defaults(func=run_claim)


def build_parser(*, prog: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog=prog, description="AWS dangling-hostname claimability validator")
    add_command_parsers(parser)
    return parser


def add_provider_parser(subcommands: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    parser = subcommands.add_parser("aws", help="Validate AWS dangling-hostname claimability")
    add_command_parsers(parser)
...
def run_claim(args: argparse.Namespace) -> int:
    ...
    def on_cleanup(current_result: dict[str, Any]) -> None:
        for payload in cleanup_payloads(current_result):
            if args.json:
                emit(json.dumps(payload, sort_keys=True))
            else:
